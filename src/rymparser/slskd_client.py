...
def wait_for_downloads(
    ...
    *,
    ...
    """Wait for all downloads from given users to finish.

    Polls slskd transfers API until every file from the
    specified users reaches a completed state.
    ...
    Returns:
        True if all downloads completed, False on timeout.
    """
    deadline = time.time() + timeout
    while time.time() < deadline:
        transfers = get_downloads(client)
        total = 0
        done = 0
        for t in transfers:
        ...
        if total > 0 and done >= total:
            logger.info(
                "All downloads complete: %d/%d files",
                done,
                total,
            )
            return True
        ...
        logger.info(
            "Downloads: %d/%d files complete, waiting...",
            done,
            total,
        )
    ...
    logger.warning(
        "Download wait timed out after %ds",
        timeout,
    )
    return False
